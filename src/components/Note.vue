<template>
  <v-card
    class="black pa-2"
    elevation="5"
    outlined
    shaped
    max-width="600"
  >
    <v-card-title class="white--text font-weight-bold">Title note</v-card-title>
    <v-card-text class="white--text font-weight-regular">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ab consectetur doloremque ea earum error fugit quae tempore voluptate. Beatae culpa dolores harum iusto non placeat. Accusamus necessitatibus nihil odio perspiciatis.</v-card-text>
    <div class="d-flex justify-end">
      <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M19 8L15 12H18C18 15.31 15.31 18 12 18C10.99 18 10.03 17.75 9.2 17.3L7.74 18.76C8.97 19.54 10.43 20 12 20C16.42 20 20 16.42 20 12H23L19 8ZM6 12C6 8.69 8.69 6 12 6C13.01 6 13.97 6.25 14.8 6.7L16.26 5.24C15.03 4.46 13.57 4 12 4C7.58 4 4 7.58 4 12H1L5 16L9 12H6Z" fill="white"/>
      </svg>
      <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M6 19C6 20.1 6.9 21 8 21H16C17.1 21 18 20.1 18 19V7H6V19ZM8 9H16V19H8V9ZM15.5 4L14.5 3H9.5L8.5 4H5V6H19V4H15.5Z" fill="white"/>
      </svg>
    </div>
  </v-card>
</template>

<script>
export default {
  name: 'Note',
};
</script>
